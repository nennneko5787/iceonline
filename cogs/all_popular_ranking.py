...
class allPopularRankingCog(commands.Cog):
    ...
    @app_commands.command(
        name="apranking",
        description=app_commands.locale_str("累積人気度ランキングを確認します。"),
    )
    @app_commands.rename(
    ...
    @app_commands.choices(
        country=[
            app_commands.Choice(name=app_commands.locale_str("日本"), value="1101"),
            app_commands.Choice(name=app_commands.locale_str("韓国"), value="1001"),
        ],
    )
    async def allPopularRankingCommand(
        ...
        *,
        ...
        view.add_item(
            discord.ui.Button(
                ...
                emoji="⏪",
                custom_id=f"apranking,{interaction.user.id},{page - 3}",
                disabled=(page <= 3),
            )
        )
        view.add_item(
...
async def setup(bot: commands.Bot):
    await bot.add_cog(allPopularRankingCog(bot))
