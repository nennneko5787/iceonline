...
class FLFRankingCog(commands.Cog):
    ...
    @app_commands.command(
        ...
        description=app_commands.locale_str(
            "釣りリーグシーズン(FLF)ランキングを確認します。"
        ),
    )
    async def FLFRankingCommand(self, interaction: discord.Interaction):
    ...
    async def responseGameRanking(
        ...
        *,
        ...
        response = await self.client.post(
            ...
            json={"ranker_page": str(page), "ranker_type": "2000"},
        )
...
async def setup(bot: commands.Bot):
    await bot.add_cog(FLFRankingCog(bot))
